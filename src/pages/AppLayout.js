...
const AppLayout = () => {
  ...
  const [permissionsLoaded, setPermissionsLoaded] = useState(false);
  const [profileLoaded, setProfileLoaded] = useState(false);
  const [initialRedirectDone, setInitialRedirectDone] = useState(false); // NEW: Track initial redirect
  ...
  // IMPROVED: Handle unauthorized access
  useEffect(() => {
    if (permissionsLoaded && roll === "user") {
  ...
  }, [location.pathname, permissionsLoaded, userPermissions, roll]);

  // IMPROVED: Load initial data with proper sequencing
  useEffect(() => {
    const loadInitialData = async () => {
      await fetchGarageProfile();
      await fetchUserPermissions();
      
      // FIXED: Only redirect on initial load, not on every navigation
      if (roll === "user" && !initialRedirectDone && location.pathname !== "/") {
        console.log("Initial redirect for user to dashboard");
  ...
  }, [garageId, roll, userId]); // Removed navigate from dependencies
  ...
  // IMPROVED: Show loading state while permissions are loading
  if (!permissionsLoaded || !profileLoaded) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ ml: 2 }}>
          {!profileLoaded ? "Loading profile..." : "Loading permissions..."}
        </Typography>
      </Box>
    );
  }
  ...
  const drawerContent = (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <Box sx={{ flexGrow: 1, p: 2, overflowY: "auto" }}>
        ...
        {/* DEBUG: Show permissions info for users */}
        {/* {roll === "user" && process.env.NODE_ENV === 'development' && (
          <Box sx={{ mt: 2, p: 1, bgcolor: 'action.hover', borderRadius: 1 }}>
            <Typography variant="caption" color="text.secondary">
              Debug - Permissions: {userPermissions.join(', ')}
            </Typography>
          </Box>
        )} */}
      </Box>
        ...
        <Toolbar>
          ...
          <Typography variant="h6" sx={{ flexGrow: 1, fontWeight: 600 }}>
            {location.pathname === "/" ? "Dashboard" : 
             filteredNavItems.find(item => item.path === location.pathname)?.text || "Page"}
          </Typography>
