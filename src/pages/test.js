...
import { ArrowBack as ArrowBackIcon, Edit as EditIcon, Close as CloseIcon } from '@mui/icons-material';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
...
const InventoryManagement = () => {
  ...
  const fetchInventory = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`https://garage-management-zi5z.onrender.com/api/garage/inventory/${garageId}`); 
      const data = response.data;
      const inventoryArray = Array.isArray(data) ? data : data.data || [];
      setInventoryData(inventoryArray);
      setFilteredData(inventoryArray);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sgstAmount = formData.sgstEnabled ? calculateTaxAmount(formData.pricePerUnit, formData.quantity, formData.sgstPercentage) : 0;
      const cgstAmount = formData.cgstEnabled ? calculateTaxAmount(formData.pricePerUnit, formData.quantity, formData.cgstPercentage) : 0;
      ...
      await axios.post('https://garage-management-zi5z.onrender.com/api/garage/inventory/add',  requestData, {
        headers: { 'Content-Type': 'application/json' }
      });

      setNotification({ open: true, message: 'Part added successfully!', severity: 'success' });
      setFormData({
        partNumber: '',
        partName: '',
        quantity: '',
  ...
  const handleUpdateItem = async () => {
    try {
      setIsEditSubmitting(true);
      if (!editItemData.id) throw new Error('Item ID is missing.');

      const sgstAmount = editItemData.sgstEnabled ? calculateTaxAmount(editItemData.pricePerUnit, editItemData.quantity, editItemData.sgstPercentage) : 0;
      const cgstAmount = editItemData.cgstEnabled ? calculateTaxAmount(editItemData.pricePerUnit, editItemData.quantity, editItemData.cgstPercentage) : 0;
  ...
  return (
    <Box sx={{ flexGrow: 1, mb: 4, ml: { xs: 0, sm: 35 }, overflow: 'auto', pt: 3 }}>
      ...
      {/* Only adding changes below for brevity */}

      {/* Table Header with Tax Column */}
      <TableHead>
        <TableRow
          sx={{
            backgroundColor: theme.palette.primary.main,
        ...
        >
          ...
          <TableCell>Total Price</TableCell>
          <TableCell>Tax Amount</TableCell> {/* NEW COLUMN */}
          <TableCell>Edit</TableCell>
        </TableRow>
      </TableHead>

      {/* Table Body with Tax Calculation */}
      <TableBody>
        {paginatedData.map((row, index) => {
          const sgstAmount = row.sgstEnabled
            ? calculateTaxAmount(row.pricePerUnit, row.quantity, row.sgstPercentage)
            : 0;
          const cgstAmount = row.cgstEnabled
            ? calculateTaxAmount(row.pricePerUnit, row.quantity, row.cgstPercentage)
            : 0;
          ...
          return (
            <TableRow key={row._id || row.id || `row-${index}`}>
              ...
              <TableCell>{row.partName}</TableCell>
              <TableCell>{row.quantity}</TableCell>
              <TableCell>₹{parseFloat(row.pricePerUnit).toFixed(2)}</TableCell>
              <TableCell>₹{(row.totalPrice || calculateTotalPrice(
                row.pricePerUnit, row.quantity,
                row.sgstEnabled, row.sgstPercentage,
                row.cgstEnabled, row.cgstPercentage
              )).toFixed(2)}</TableCell>
              <TableCell>₹{totalTax.toFixed(2)}</TableCell> {/* DISPLAY TAX AMOUNT */}
              <TableCell>
                <Button
                  variant="outlined"
                  color="primary"
      ...
      {/* Remaining code like pagination, summary, modals, etc., unchanged */}
    </Box>
  );
};
