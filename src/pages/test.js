...
const InventoryManagement = () => {
  ...
  // Calculate tax amount (total for quantity)
  ...
  const calculateSinglePartGST = (sellingPrice, igst, cgstSgst) => {
    ...
    const igstAmount = igst ? (parseFloat(sellingPrice) * parseFloat(igst)) / 100 : 0;
    const cgstSgstAmount = cgstSgst ? (parseFloat(sellingPrice) * parseFloat(cgstSgst) * 2) / 100 : 0;
    return igstAmount + cgstSgstAmount;
  };
  ...
  // Calculate total GST amount for all inventory items
  const calculateTotalGSTAmount = () => {
    ...
    inventoryData.forEach(item => {
      ...
      const gstAmount = calculateSinglePartGST(sellingPrice, igst, cgstSgst) * quantity;
  ...
  const handleOpenEditModal = (item) => {
    ...
    setEditItemData({
      ...
      taxType: item.igst ? 'igst' : 'cgstSgst',
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Calculate single part GST amount
      const singlePartGST = calculateSinglePartGST(
      ...
      const requestData = {
        ...
        taxAmount: parseFloat(singlePartGST.toFixed(2)), // Store single part GST
      ...
      const gstMessage = singlePartGST > 0 
        ? ` with GST: ₹${singlePartGST.toFixed(2)} per unit (${formData.igst ? `IGST: ${formData.igst}%` : `CGST+SGST: ${formData.cgstSgst}% each`})`
        : ' (No GST applied)';
      
      setNotification({
        open: true,
        message: `✅ Part "${formData.partName}" added successfully!${gstMessage}`,
        severity: 'success',
      });
  ...
  const handleUpdateItem = async () => {
    try {
      ...
      // Calculate single part GST amount for edit
      const singlePartGST = calculateSinglePartGST(
        editItemData.sellingPrice,
        editItemData.igst,
      ...
      // Show detailed success message with GST calculation for edit
      const gstMessage = singlePartGST > 0 
        ? ` with GST: ₹${singlePartGST.toFixed(2)} per unit (${editItemData.igst ? `IGST: ${editItemData.igst}%` : `CGST+SGST: ${editItemData.cgstSgst}% each`})`
        : ' (No GST applied)';
      
      setNotification({
        open: true,
        message: `✅ Part "${editItemData.partName}" updated successfully!${gstMessage}`,
        severity: 'success',
      });
    ...
    >
                    ...
                    >
                      ...
                      <TableCell>
                        <Typography variant="body2" color="primary" fontWeight={600}>
                          ₹{(row.taxAmount || calculateSinglePartGST(
                            row.sellingPrice,
                            row.igst,
                            row.cgstSgst
                          )).toFixed(2)}
                        ...
                        <Typography variant="caption" color="success.main" display="block">
                          Total: ₹{((row.taxAmount || calculateSinglePartGST(
                            row.sellingPrice,
                            row.igst,
                            row.cgstSgst
                  ...
                  <Typography variant="h6" fontWeight={600} color="primary">
                    ₹{filteredData
                      .reduce(
                        (sum, item) =>
                          sum +
                          (calculateSinglePartGST(item.sellingPrice, item.igst, item.cgstSgst) * (item.quantity || 0)),
                        0
                      )
                      .toFixed(2)}
                  </Typography>
                ...
                <Grid item xs={12} sm={3}>
                  <Typography variant="h6" fontWeight={600}>
                    ₹{filteredData
                      .reduce(
                        (sum, item) => {
                          const baseAmount = (item.sellingPrice || 0) * (item.quantity || 0);
                          const gstAmount = calculateSinglePartGST(item.sellingPrice, item.igst, item.cgstSgst) * (item.quantity || 0);
                ...
                return (
                  <Box sx={{ 
                    ...
                    <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'success.dark', mb: 1 }}>
                      💰 GST Summary (All Inventory Items):
                    </Typography>
                    ...
                    <Typography variant="caption" color="success.dark" sx={{ mt: 1, display: 'block', fontStyle: 'italic' }}>
                      ✅ GST calculation based on selling price and tax percentages
                    </Typography>
                  </Box>
                );
              }
      ...
      <Dialog open={addModalOpen} onClose={handleCloseAddModal} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          <Box component="form" onSubmit={handleSubmit}>
            ...
            {formData.sellingPrice && formData.quantity && (formData.igst || formData.cgstSgst) && (
              <Box sx={{ mt: 3, p: 2, bgcolor: 'action.hover', borderRadius: 1 }}>
                <Typography variant="h6" sx={{ mb: 2 }}>GST Calculation Preview</Typography>
                <Grid container spacing={2}>
                                     <Grid item xs={12} sm={6}>
                     <Typography variant="body2" color="text.secondary">
                       Selling Price per Unit: ₹{parseFloat(formData.sellingPrice || 0).toFixed(2)}
                     </Typography>
                   </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2" color="text.secondary">
                  ...
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2" color="primary">
                      Single Part GST: ₹{calculateSinglePartGST(
                        formData.sellingPrice,
                        formData.igst,
                        formData.cgstSgst
                      ).toFixed(2)}
                  ...
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2" color="success.main">
                      Total GST for Quantity: ₹{(calculateSinglePartGST(
                        formData.sellingPrice,
                        formData.igst,
                        formData.cgstSgst
                      ) * parseInt(formData.quantity)).toFixed(2)}
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Typography variant="h6" color="success.dark">
                      Total Amount (Including GST): ₹{(() => {
                        ...
                        const totalAmount = (parseFloat(formData.sellingPrice) + singlePartGST) * parseInt(formData.quantity);
      ...
      <Dialog open={editModalOpen} onClose={handleCloseEditModal} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          ...
          {/* GST Calculation Preview for Edit */}
          {editItemData.sellingPrice && editItemData.quantity && (editItemData.igst || editItemData.cgstSgst) && (
            <Box sx={{ mt: 3, p: 2, bgcolor: 'action.hover', borderRadius: 1 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>GST Calculation Preview</Typography>
              <Grid container spacing={2}>
                                 <Grid item xs={12} sm={6}>
                   <Typography variant="body2" color="text.secondary">
                     Selling Price per Unit: ₹{parseFloat(editItemData.sellingPrice || 0).toFixed(2)}
                   </Typography>
                 </Grid>
                <Grid item xs={12} sm={6}>
                  <Typography variant="body2" color="text.secondary">
                ...
                <Grid item xs={12} sm={6}>
                  <Typography variant="body2" color="success.main">
                    Total GST for Quantity: ₹{(calculateSinglePartGST(
                      ...
                      editItemData.igst,
                      editItemData.cgstSgst
                    ) * parseInt(editItemData.quantity)).toFixed(2)}
                  </Typography>
                </Grid>
                <Grid item xs={12}>
                  <Typography variant="h6" color="success.dark">
                    Total Amount (Including GST): ₹{(() => {
                      ...
                      const totalAmount = (parseFloat(editItemData.sellingPrice) + singlePartGST) * parseInt(editItemData.quantity);
                      return totalAmount.toFixed(2);
                    })()}
                  </Typography>
                </Grid>
