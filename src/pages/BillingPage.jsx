...
const AutoServeBilling = () => {
      ...
      try {
        ...
        setGarageDetails({
          ...
          bankDetails: {
            ...
            accountNumber:
              data.bankDetails?.accountNumber || data.accountNumber || "",
      ...
      try {
        ...
        if (data.partsUsed?.length > 0) {
          const apiParts = data.partsUsed.map((part, index) => ({
            ...
            pricePerUnit:
              parseFloat(
                part.pricePerPiece || part.pricePerUnit || part.sellingPrice
              ) || 0,
            total:
              parseFloat(part.totalPrice) ||
              parseInt(part.quantity) *
                parseFloat(part.pricePerPiece || part.sellingPrice) ||
              0,
          ...
          try {
            ...
            if (Array.isArray(jobDetailsArray) && jobDetailsArray.length > 0) {
              apiServices = jobDetailsArray.map((job, index) => ({
                ...
                engineer:
                  (data.engineerId?.length > 0
                    ? data.engineerId[0].name
                    : null) ||
                  data.engineerId?.name ||
                  "Assigned Engineer",
                progress: 100,
        ...
        setSnackbar({
          open: true,
          message: `Error: ${
            error.response?.data?.message || "Failed to fetch job card data"
          }`,
          severity: "error",
        });
      } finally {
  ...
  const handleCustomerGstChange = (event) => {
    setGstSettings((prev) => ({
      ...prev,
      customerGstNumber: event.target.value.toUpperCase(),
    }));
  };
  ...
  const testApiEndpoints = async () => {
    ...
    for (const endpoint of endpointsToTest) {
      try {
        ...
        const response = await axios.get(
          endpoint.replace("generate", "test").replace("create", "test"),
          {
            headers: {
        ...
        if (error.response) {
          console.log(
            `❓ ${endpoint} - Status: ${error.response.status} (${
              error.response.status === 404 ? "Not Found" : "Other Error"
            })`
          );
        } else {
    ...
    try {
      const healthCheck = await axios.get(
        "https://garage-management-zi5z.onrender.com/api/health",
        {
          timeout: 5000,
        }
      );
  ...
  const validateJobCompletion = () => {
    ...
    if (!hasPartsOrServices && totalAmount === 0) {
      issues.push(
        "At least one part or service is required, or add a minimum service charge"
      );
    }
    ...
    if (!gstSettings.billToParty.trim()) {
      ...
      if (carDetails.customerName.trim()) {
        setGstSettings((prev) => ({
          ...prev,
          billToParty: carDetails.customerName,
        }));
      } else {
  ...
  const ensureMinimumBillRequirements = () => {
    ...
    if (updated) {
      setSnackbar({
        open: true,
        message:
          "Default values added to complete the bill. Please review before generating.",
        severity: "info",
      });
    }
  ...
  const processPayment = async () => {
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          setApiResponseMessage({
            ...
            message:
              data.message ||
              "Professional bill generated and payment processed successfully! Job completed.",
          });
          setShowThankYou(true);
    ...
    if (!success) {
      try {
        ...
        setApiResponseMessage({
          type: "error",
          message: `Bill generation failed. Last error: ${
    ...
    try {
      ...
      setApiResponseMessage({
        ...
        message:
          ...
          "Bill generation failed. Please contact support.",
      });
      setShowApiResponse(true);
    }
  };
  ...
  const processOnlinePayment = async () => {
    ...
    try {
      ...
      if (
        !billResponse ||
        (billResponse.status !== 200 && billResponse.status !== 201)
      ) {
        await handleFallbackBillGeneration(apiData);
        ...
        setApiResponseMessage({
          type: "warning",
          message:
            "Bill generated using fallback method. Online payment may need manual processing.",
        });
        setShowThankYou(true);
        setShowApiResponse(true);
        return;
      ...
      if (responseData.bill && responseData.bill._id) {
        ...
        try {
          ...
          if (
            ...
            setApiResponseMessage({
              ...
              message:
                ...
                "Online payment processed successfully! Job completed.",
            });
            setShowThankYou(true);
          } else {
          ...
          setApiResponseMessage({
            type: "warning",
            message:
              "Bill generated successfully, but online payment processing failed. Please process payment manually.",
          });
          setShowThankYou(true);
        }
      } else {
        ...
        setApiResponseMessage({
          type: "warning",
          message:
            "Bill generated but payment processing unavailable. Please process payment manually.",
        });
        setShowThankYou(true);
      }
    } catch (error) {
  ...
  const generateProfessionalGSTInvoice = () => {
    try {
      ...
      // Bill Type Section
      const billTypeText =
        gstSettings.billType === "gst" ? "GST Tax Invoice" : "Non-GST Invoice";
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `Name: ${gstSettings.billToParty || carDetails.customerName}`,
        margin + 10,
        billShipY + 40
      );
      ...
      if (gstSettings.customerGstNumber && gstSettings.billType === "gst") {
        doc.text(
          `GST No: ${gstSettings.customerGstNumber}`,
          margin + 10,
          billShipY + 85
        );
      }
      ...
      doc.text(
        "Ship To / Insurance:",
        margin + billToWidth + 20,
        billShipY + 20
      );
      doc.setFont("helvetica", "normal");
      doc.text(
        `Party Name: ${gstSettings.insuranceProvider || "N/A"}`,
        margin + billToWidth + 20,
        billShipY + 40
      );
      doc.text(
      ...
      const totalTableWidth = Object.values(colWidths).reduce(
        (sum, width) => sum + width,
        0
      );
      ...
      doc.text(
        Math.round(summary.totalAmount).toFixed(2),
        summaryX + summaryWidth - 80,
        currentY + 20
      );
      currentY += 40;
      ...
      // Amount in Words
      const amountInWords =
        numberToWords(Math.round(summary.totalAmount)) + " Only";
      ...
      // UPDATED: Bank Details Section with complete information
      if (
        garageDetails.bankDetails.bankName ||
        garageDetails.bankDetails.accountNumber
      ) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          `Bank: ${garageDetails.bankDetails.bankName || "N/A"}`,
          margin + 10,
          currentY + 35
        );
        doc.text(
          `A/c Holder: ${garageDetails.bankDetails.accountHolderName || "N/A"}`,
          margin + 10,
          currentY + 50
        );
        doc.text(
          `A/c No: ${garageDetails.bankDetails.accountNumber || "N/A"}`,
          margin + 10,
          currentY + 65
        );
        doc.text(
          `IFSC: ${garageDetails.bankDetails.ifscCode || "N/A"}`,
          margin + 10,
          currentY + 80
        );
        if (garageDetails.bankDetails.upiId) {
          doc.text(
            `UPI: ${garageDetails.bankDetails.upiId}`,
            margin + 10,
            currentY + 95
          );
        }
      ...
      doc.text(
        "2. Our risk ceases as goods leave premises.",
        termsX + 10,
        currentY + 47
      );
      doc.text(
      ...
      doc.text(
        `Bill Type: ${gstSettings.billType.toUpperCase()}`,
      ...
      throw new Error(
        "Failed to generate professional invoice: " + error.message
      );
    }
  };
  ...
  const downloadPdfBill = () => {
    try {
      ...
      const fileName = `${billTypeText}_Invoice_${
        carDetails.invoiceNo
      }_${carDetails.carNumber.replace(/[^a-zA-Z0-9]/g, "_")}_${
  ...
  const sendBillViaEmail = async () => {
    try {
      ...
      const billTypeText = gstSettings.billType === "gst" ? "GST" : "Non-GST";
      downloadLink.download = `${billTypeText}_Invoice_${
        carDetails.invoiceNo
      }_${carDetails.carNumber.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`;
      downloadLink.click();

      const subject = encodeURIComponent(
        emailSubject ||
          `${billTypeText} Invoice #${carDetails.invoiceNo} - ${garageDetails.name}`
      );
      const gstInfo =
        gstSettings.billType === "gst"
    ...
    try {
      ...
      if (gstSettings.billType === "gst") {
        if (gstSettings.isInterState) {
          ...
          }%): ₹${Math.round(summary.gstAmount / 2)}\n🔸 SGST (${
...
${
  parts.length > 0
    ? `*PARTS USED:*\n${parts
        .map(
          (part) =>
            `🔧 ${part.name} (${part.quantity}) - HSN: ${part.hsnNumber}: ₹${part.total}`
        )
        .join("\n")}\n`
    : ""
}
${
  services.length > 0
    ? `*SERVICES PROVIDED:*\n${services
        .map((service) => `⚙️ ${service.name}: ₹${service.laborCost}`)
        .join("\n")}\n`
    : ""
}
━━━━━━━━━━━━━━━━━━━━━
...
*Visit us again for quality automotive care.*`;
      ...
      setApiResponseMessage({
        type: "success",
        message: `WhatsApp ${gstSettings.billType.toUpperCase()} invoice prepared for ${
      ...
      <Paper elevation={3} sx={{ mb: 4, p: isMobile ? 2 : 3, borderRadius: 2 }}>
        ...
        {isBillAlreadyGenerated && (
          <Alert severity="info" sx={{ mb: 3 }}>
            This {gstSettings.billType.toUpperCase()} invoice has already been
            generated. You can download the PDF or share via WhatsApp/Email
            below.
          </Alert>
        )}
