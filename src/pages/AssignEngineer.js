...
// Configuration - centralize API base URL
...
const AssignEngineer = () => {
  ...
  // Fetch inventory parts from API
  useEffect(() => {
    const fetchInventory = async () => {
      if (!garageId) {
        navigate("/login"); // Fixed slash direction
        return;
      }
      
      try {
        const response = await apiCall(`/garage/inventory/${garageId}`, {
          method: 'GET'
        });
    ...
    fetchInventory();
  }, [garageId, partAddSuccess, navigate]); // Fixed dependencies
  ...
  useEffect(() => {
    const fetchEngineers = async () => {
      ...
      try {
        ...
        const response = await apiCall(`/garage/engineers/${garageId}`, {
          method: 'GET'
        });
    ...
    fetchEngineers();
  }, [garageId, engineerAddSuccess, navigate]); // Fixed dependencies
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/garage/jobCards/assign-engineer/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ 
          ...
          parts: selectedParts.map(part => part._id) // Include selected parts
        })
      });
  ...
  // Dialog handlers
  const handleOpenAddPartDialog = () => setOpenAddPartDialog(true);
  const handleCloseAddPartDialog = () => {
    setOpenAddPartDialog(false);
    setPartAddError(null);
    setNewPart({
  ...
  const handleOpenAddEngineerDialog = () => setOpenAddEngineerDialog(true);
  const handleCloseAddEngineerDialog = () => {
    setOpenAddEngineerDialog(false);
    setEngineerAddError(null);
    setNewEngineer({
  ...
  const handlePartInputChange = (e) => {
    ...
    const numericFields = ['quantity', 'pricePerUnit', 'taxAmount'];
    setNewPart(prev => ({
      ...prev,
  ...
  const handleAddEngineer = async () => {
    ...
    if (!/^\d{10}$/.test(newEngineer.phone)) {
      setEngineerAddError('Please enter a valid 10-digit phone number');
      return;
    }
  ...
  const handleAddPart = async () => {
    ...
    if (newPart.quantity <= 0 || newPart.pricePerUnit < 0) {
      setPartAddError('Quantity must be greater than 0 and price cannot be negative');
      return;
    }
