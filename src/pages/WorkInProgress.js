...
const WorkInProgress = () => {
  ...
  const addInventoryPartToList = (inventoryPart) => {
    ...
    const newPart = {
      ...
      gstPercentage:
        inventoryPart.gstPercentage || inventoryPart.taxAmount || 0,
      ...
      try {
        ...
        if (data.partsUsed && data.partsUsed.length > 0) {
          const existingParts = data.partsUsed.map((part, index) => ({
            ...
            quantity: part.quantity || 1, // Add quantity field for tax calculations
            taxAmount: part.gstPercentage || 0, // Add taxAmount field
          }));

          setAllParts(existingParts);
          setSelectedParts(existingParts); // FIX: Load existing parts into selectedParts
          setPartIdCounter(existingParts.length + 1);
        } else {
          // Initialize with empty array if no existing parts
          setAllParts([]);
          setSelectedParts([]); // FIX: Initialize selectedParts as empty
          setPartIdCounter(1);
        }
    ...
    try {
      ...
      for (const part of inventoryParts) {
        ...
        const currentPart = inventoryParts.find(
          (p) => p._id === part.inventoryId
        );
        if (currentPart) {
          const newQuantity = currentPart.quantity - selectedQuantity;
          if (newQuantity < 0) {
            throw new Error(
              `Insufficient stock for "${part.partName}". Required: ${selectedQuantity}, Available: ${currentPart.quantity}`
            );
          }
                      ...
                      <Box sx={{ mt: 2 }}>
                        ...
                        <List dense>
                          {selectedParts.map((part, partIndex) => {
                            ...
                            const totalTax =
                              (selectedQuantity * taxAmount) / quantity;
                    ...
                    {allParts.length === 0 ? (
                      ...
                      >
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Existing parts will be loaded from job card or add new
                          inventory parts
                        </Typography>
                      </Box>
                    ) : (
                                    ...
                                    >
                                      <IconButton
                                        ...
                                        onClick={() => {
                                          ...
                                          if (newQuantity >= 1) {
                                            handleInventoryPartQuantityChange(
                                              part.id,
                                              newQuantity
                                            );
                                          }
